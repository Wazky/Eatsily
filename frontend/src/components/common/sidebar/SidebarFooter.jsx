...
export default function SidebarFooter() {
    ...
    const initials = (user) ? upperFirst(user.username).slice(0, 2).toUpperCase() : t('sidebar.user.guest.initials');
    const userName = (user) ? upperFirst(user.username) : t('sidebar.user.guest.name');
    const userEmail = (user) ? user.email : t('sidebar.user.guest.email');

        
    return (
    <>
        {isCollapsed && (
        <div className="position-relative"> {/* Contenedor relativo para posicionamiento absoluto */}
            ...
            {/* Hover text - posicionado absolutamente */}
            <span className="sidebar-hover-text bg-primary-400 aa-txt-text">
                {upperFirst(t('sidebar.actions.logout'))}
            </span>
        </div>
        )}
                ...
                <Link to={ROUTES.PROFILE} className="d-flex align-items-center text-decoration-none py-0 gap-2">
                    ...
                    {!isCollapsed && (
                        <div className="text-start">
                            <p className="mb-0 fw-bold hover-bright-text">
                                {userName}
                            </p>
                            <p className="mb-0 hover-bright-text">
                                {userEmail}
                            </p>
                        </div>                    
                    )}
            ...
            {!isCollapsed && (
                <div className="w-100">    
                    ...
                    <div className="d-flex flex-row justify-content-end p-1 mt-0">
                        <span className="small">© 2026 EATSILY</span>
                    </div>
                </div>
            )}
